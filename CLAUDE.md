...
To test the login page on a real phone over the same WiFi (not just browser devtools' mobile emulation), `http://localhost:8000` in `.env.local` won't work — it's baked into the JS bundle at build time and resolves to the phone itself, not your machine, producing a network error on the phone (but not on your laptop, where `localhost` correctly means "this machine"). Use your computer's LAN IP instead: `NEXT_PUBLIC_API_URL=http://<your-LAN-IP>:8000`, restart `npm run dev`, and add that same IP to the backend's `ALLOWED_HOSTS` and `CORS_ALLOWED_ORIGINS` env vars (both default to `localhost`/`127.0.0.1` only).
...
#### RBAC pattern
Every viewset sets `permission_classes = [IsAuthenticated]`; row-level scoping is done via `get_queryset()` checking `request.user.role`. The reference implementation is `StudentViewSet.get_queryset()` (`students/views.py`): students see only their own record, guardians see linked students (`guardians__user=user`), teachers see students in classes they're assigned to (via `teacher.assignments.filter(is_active=True)`), admins see everything. `GuardianViewSet` follows the same pattern.

`apps.users.views.scope_to_own_student(qs, user, prefix="student", teacher_sees_classes=True)` is the shared helper for everything that hangs off a `Student` FK — `marks`, `attendance`, and `finance` viewsets all call it from `get_queryset()` (passing a `prefix` for indirect relations, e.g. `payment__invoice__student` on `ReceiptViewSet`). It returns `.none()` for `finance_officer` by default ("no access to academic records") and for `teacher` when `teacher_sees_classes=False` ("no access to financial records") — so any finance viewset that needs `finance_officer` to see everything (as `InvoiceViewSet`/`PaymentViewSet`/`ReceiptViewSet` do) must short-circuit with `if user.role in ("admin", "finance_officer"): return super().get_queryset()` *before* falling through to `scope_to_own_student()`, the way `InvoiceViewSet` does — forgetting that short-circuit silently locks finance officers out (this exact bug existed on `ReceiptViewSet` and was fixed).

`teachers` viewsets (`TeacherViewSet`, `TeacherAssignmentViewSet`) intentionally have no row-level scoping — any authenticated role can list the staff directory and assignments; only mutations are admin-gated. Reference/structural data (`Class`, `Subject`, `AcademicYear`, `Semester`, `GradingScale`, `ConductCategory`) is likewise world-readable to any authenticated user, admin-gated only for writes.
